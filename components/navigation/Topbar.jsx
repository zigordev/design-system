  ...
  return (
    <header className={`ds-topbar ${className}`.trim()} style={style}>
      ...
      >
        {brand ? (
          <div
            className={`ds-topbar-brand ${hideBrandOnDesktop ? 'ds-topbar-brand--hide-desktop' : ''}`.trim()}
          >
            {brand}
          </div>
        ) : null}
        ...
        {hasHeaderText ? (
          <div
            className="ds-topbar-header-text"
            style={{ display: 'grid', gap: 2, minWidth: 0, flex: '1 1 auto' }}
          >
            {title ? (
              <h1
                style={{
        ...
        {mode ? (
          <div style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>{mode}</div>
        ) : null}
        {actions ? (
          <div
            style={{
