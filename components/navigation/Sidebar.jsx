...
function SidebarItem({ item, activeHref, linkComponent }) {
  ...
  return (
    <div>
      ...
      >
        {item.icon ? (
          <span aria-hidden="true" style={{ display: 'grid', placeItems: 'center', width: 18 }}>
            {item.icon}
          </span>
        ) : null}
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  fontWeight: childActive ? 'var(--ds-weight-semibold)' : 'var(--ds-weight-medium)',
...
export function Sidebar({
  ...
  return (
    ...
    >
      ...
      {scope ? <div style={{ padding: '12px 12px 0' }}>{scope}</div> : null}
      <nav
        style={{
          flex: 1,
          overflowY: 'auto',
      ...
      {footer ? (
        <div
          style={{ borderTop: '1px solid var(--ds-color-border)', padding: 'var(--ds-space-4)' }}
        >
          {footer}
        </div>
      ) : null}
