Brand mark used as `Sidebar`'s `brand` slot and standalone in `Topbar`/login screens. No product here ships a real logo asset — pass either `initials` (a letter or pair) or an arbitrary `mark` icon node; never fabricate new artwork. `tagline` only renders at `size="lg"`. When `href` is set, pass `linkComponent` (e.g. `next/link`'s `Link`) in a router-based app — the default plain `<a>` causes a full page reload on click. Without a `wordmark`, the link has no visible text, so pass `label` (the product name) to give it an accessible name; it falls back to `initials`.
